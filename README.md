...
downloads the New York OSM extract, clips it to a Capital Region bbox around
Niskayuna/Schenectady/Albany, prebuilds that Valhalla graph during Cloud Build,
and starts a small proxy on Cloud Run's injected port. After the Cloud Run
service is healthy, set Netlify `VALHALLA_BASE_URL` to the service URL without a
path suffix. Set the same `VALHALLA_SHARED_SECRET` value in Netlify and Cloud
Run if the direct Cloud Run URL should reject public `/isochrone` calls. See
`cloudrun/README.md` for the full service settings.
